...
export function HowToUseSection() {
    ...
    return (
        <section id="how-it-works" className="py-12 md:py-24 relative px-4 overflow-hidden">
            ...
            <div className="relative z-10 max-w-6xl mx-auto">
                ...
                <div className="text-center max-w-2xl mx-auto mb-12 md:mb-16">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-900/50 border border-white/10 text-xs mb-4">
                        <Sparkles className="w-3 h-3 text-violet-400" />
                        <span className="text-slate-300">Simple Workflow</span>
                    </div>
                ...
                <div className="md:hidden relative max-w-sm mx-auto pl-4">
                    ...
                    <div className="absolute left-[27px] top-6 bottom-6 w-0.5 bg-gradient-to-b from-violet-500 via-cyan-500 to-amber-500 opacity-30" />
                        ...
                        {steps.map((step, index) => {
                            ...
                            return (
                                <div key={index} className="relative flex items-center gap-6 group">
                                    {/* Flow Node */}
                                    <div className={`relative z-10 w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 border-2 transition-all duration-300 bg-slate-900 ${colors.border} shadow-lg shadow-black/50`}>
                                        ...
                                        {/* Step Number Badge */}
                                        <div className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-white text-slate-900 text-[10px] font-bold flex items-center justify-center border-2 border-slate-900">
                                            {index + 1}
                                        </div>
                                    </div>

                                    {/* Content Card */}
                                    <div className="flex-1 p-4 rounded-xl bg-slate-900/40 border border-white/5 backdrop-blur-sm hover:bg-slate-900/60 transition-colors">
                                        <h3 className="text-white font-bold text-base mb-1">{step.title}</h3>
                                        <p className="text-slate-400 text-xs leading-relaxed">{step.mobileDesc}</p>
                                    </div>

                                    {/* Connecting Pulse (Visual Flair) */}
                                    {index !== steps.length - 1 && (
                                        <div className={`absolute left-[26px] -bottom-6 w-1 h-3 rounded-full ${colors.line} opacity-50`} />
                                    )}
                                </div>
                            );
                        })}
                ...
                <div className="hidden md:grid grid-cols-2 gap-12 items-center">
                    ...
                    <div className="relative">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                            <div className={cn(
                                "w-96 h-96 rounded-full border-2 opacity-20 transition-colors duration-500",
